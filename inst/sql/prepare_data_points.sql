...
FROM platform.table_dimensions t
WHERE id = any(dim_id)
order by 1);
...
SELECT
    ...
    dimz.dimension
FROM
    platform.series_levels
    LEFT JOIN (
        SELECT
            id,
            table_dimensions.dimension
        FROM
            platform.table_dimensions
        WHERE
            table_id = (
            SELECT id
            FROM platform.table
...
-- #                             sprintf("SELECT series_id, level_value, j.dimension FROM platform.series_levels left join
...
--                                     (select *
--                                     from crosstab(
--                                     'SELECT series_id,  j.dimension, level_value FROM platform.series_levels
--                                     left join
--                                     (SELECT id, dimension FROM platform.table_dimensions
--                                      where id in (%s)) as j
--                                     on tab_dim_id = j.id
...
CREATE OR REPLACE FUNCTION platform.f_dynamic_copy(_file    text
                                        , _tbl     text = 'tmp1'
                                        , _delim   text = E'\t'
